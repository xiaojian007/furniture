...
class SpiderAnimation {
	...
	init(params) {
		...
		if (!params.direction) {
			this.init(Object.assign(params, {direction: 'left'}))
			this.init(Object.assign(params, {direction: 'right'}))
			return
		}
		var randomLines = [], {
			el,
		...
		!function () {
			...
			function draw_canvas() {
				...
				//遍历处理每一个点
				randomLines.forEach(function (r, idx) {
					r.x += r.xa
					r.y += r.ya //移动 
					if (direction === 'left') {
						r.xa *= r.x + r.y >= width || r.x < 0 || r.x > canvas_width ? -1 : 1
						r.ya *= r.x + r.y >= width || r.y < 0 || r.y > canvas_height ? -1 : 1
					} else {
					...
					context.arc(r.x - 0.5, r.y - 0.5, r.width, 0, Math.PI * 2, true)
					context.fill();
			...
			div.style.cssText = `width: ${width}px;height: ${width}px;position: absolute;${direction}: 0;top: 0;z-index: 1;`
			...
			//随机生成config.n条线位置信息
			var i
			for (randomLines = [], i = 0; config.n > i; i++) {
				var x = random() * width,
			...
			all_array = randomLines.concat([current_point]);
			//0.1秒后绘制
			setTimeout(function () {
				draw_canvas();
			}, 0);
